...
def test_track_record_without_siblings_is_still_usable() -> None:
    ...
    for siblings in unusable:
        track = parse_track_record(_provider(), _CATALOGUE["TR:100"], "TR:100", siblings)
        assert track.item_id == "TR:100"
        assert track.name == "Some Song"
...
def test_album_record_without_siblings_is_still_usable() -> None:
    ...
    for siblings in unusable:
        album = parse_album_record(_provider(), record, "AL:157378", siblings)
...
def test_station_is_a_dynamic_radio() -> None:
    """A station parses as a dynamic radio station keeping its Pandora id."""
    station = parse_station(_provider(), {"stationId": "ST:1", "name": "Some Radio"})
    assert isinstance(station, Radio)
    assert station.is_dynamic
    assert station.item_id == "ST:1"
