...
def test_flags_are_returned_as_a_set() -> None:
    ...
    response = {"config": {"flags": ["onDemand", "adFreeSkip"]}}
    assert read_account_flags(response) == {"onDemand", "adFreeSkip"}
...
def test_free_account_flags_do_not_include_on_demand() -> None:
    """Measured free-tier payload: ad-supported replay/skip, and no onDemand."""
    response = {"config": {"flags": ["adSupportedReplay", "adSupportedSkip"]}}
    assert "onDemand" not in read_account_flags(response)
