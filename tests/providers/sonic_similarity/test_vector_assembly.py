...
def _make_analysis(**overrides: object) -> AudioAnalysisData:
    defaults: dict[str, object] = {
        ...
        "key": "C",
        "mode": "major",
        "rms_energy_per_second": np.array([0.1, 0.2, 0.15, 0.18], dtype=np.float32),
        "spectral_centroid_per_second": np.array(
            [2000.0, 2200.0, 2100.0, 2050.0], dtype=np.float32
        ),
    }
...
class TestAssembleVector:
    ...
    def test_key_mode_at_indices_9_10_11(self) -> None:
        """Indices 9, 10, 11 hold key_sin, key_cos, mode."""
        analysis = _make_analysis(key="C", mode="major")
        result = assemble_vector(analysis)
        assert result is not None
        expected_sin, expected_cos, expected_mode = encode_key_mode("C", "major")
        assert result[9] == pytest.approx(expected_sin, abs=1e-9)
        assert result[10] == pytest.approx(expected_cos, abs=1e-9)
        assert result[11] == pytest.approx(expected_mode)

    def test_rms_variance_at_index_12(self) -> None:
        """Index 12 holds variance of rms_energy_per_second."""
        rms = np.array([0.1, 0.2, 0.15, 0.18], dtype=np.float32)
        analysis = _make_analysis(rms_energy_per_second=rms)
        result = assemble_vector(analysis)
        assert result is not None
        assert result[12] == pytest.approx(float(np.var(rms)), rel=1e-5)

    def test_centroid_variance_at_index_13(self) -> None:
        """Index 13 holds variance of spectral_centroid_per_second."""
        centroid = np.array([2000.0, 2200.0, 2100.0, 2050.0], dtype=np.float32)
        analysis = _make_analysis(spectral_centroid_per_second=centroid)
        result = assemble_vector(analysis)
        assert result is not None
        assert result[13] == pytest.approx(float(np.var(centroid)), rel=1e-5)

    def test_rms_variance_defaults_to_zero_when_none(self) -> None:
        """rms_energy_per_second=None → variance index 12 is 0.0."""
        analysis = _make_analysis(rms_energy_per_second=None)
        result = assemble_vector(analysis)
        assert result is not None
        assert result[12] == pytest.approx(0.0)

    def test_centroid_variance_defaults_to_zero_when_none(self) -> None:
        """spectral_centroid_per_second=None → variance index 13 is 0.0."""
        analysis = _make_analysis(spectral_centroid_per_second=None)
        result = assemble_vector(analysis)
        assert result is not None
        assert result[13] == pytest.approx(0.0)

    def test_rms_variance_defaults_to_zero_for_single_element(self) -> None:
        """rms_energy_per_second with length 1 → variance index 12 is 0.0."""
        analysis = _make_analysis(rms_energy_per_second=np.array([0.5], dtype=np.float32))
        result = assemble_vector(analysis)
        assert result is not None
        assert result[12] == pytest.approx(0.0)
...
class TestNormalizeFeatures:
    ...
    def test_unit_length_after_normalization(self) -> None:
        ...
        raw = [120.0, 0.7, 0.6, -14.0, 8.0, 0.5, 0.4, 0.3, 0.8, 0.0, 1.0, 1.0, 0.01, 5000.0]
        means = raw[:]  # All z-scores will be 0 unless we shift
        stds = [1.0] * VECTOR_DIMENSIONS
        # Use different means to get non-zero z-scores
        means2 = [v + 1.0 for v in raw]
        result = normalize_features(raw, means2, stds)
